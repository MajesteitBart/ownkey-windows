...
(() => {
  ...
  function renderSummary(names) {
    ...
    const outdated = (a) => a && a.input_rev !== m.transcript_rev;
    const cites = (refs, a) => outdated(a) ? '' : (refs || []).filter((r) => times[r]).map((r) => `<button class="cite" data-cite="${r}"><span class="dot"></span>${times[r]}</button>`).join('');
    ...
    if (!d.passages.length) body = `<p class="hint">Transcribe the meeting first. Summary, questions and drafts are built on passages with timestamps.</p>`;
      ...
      const c = s.content;
      body = `${outdated(s) ? `<div class="banner">${ico('alert', 16)}<div class="body"><b>Summary is based on transcript rev ${s.input_rev}.</b><p>The transcript has changed (now rev ${m.transcript_rev}). Regenerate the summary to restore citations for the current transcript.</p></div><div class="acts"><button class="btn secondary xs" data-summary>${ico('refresh', 12)}Regenerate</button></div></div>` : ''}
        <div class="sum"><h3>Overview</h3><p class="lead">${esc(c.overview || '')}</p>
        ${c.decisions && c.decisions.length ? `<h3>Decisions <span class="cnt">${c.decisions.length}</span></h3>${c.decisions.map((x) => item(x.status === 'decided' ? 'green' : 'amber', x.status === 'decided' ? ico('check', 11, 3.5) : '!', esc(x.text), `<span class="tagd ${x.status}">${x.status}</span>${x.unverified ? '<span class="tagd unverified" title="No cited passage shares words with this claim">unverified</span>' : ''}${cites(x.refs, s)}`)).join('')}` : ''}
        ${c.actions && c.actions.length ? `<h3>Action items <span class="cnt">${c.actions.length}${c.actions.some((a) => !a.owner) ? ' · some without owner' : ''}</span></h3>${c.actions.map((x) => item('neutral', '→', esc(x.text), `${x.owner ? `<span>${esc(x.owner)}</span>` : '<span class="unassigned">Unassigned</span>'}<span>·</span>${x.due ? `<span>${esc(x.due)}</span>` : '<span class="unassigned">No date</span>'}${x.unverified ? '<span class="tagd unverified">unverified</span>' : ''}${cites(x.refs, s)}`)).join('')}` : ''}
        ${c.questions && c.questions.length ? `<h3>Open questions <span class="cnt">${c.questions.length}</span></h3>${c.questions.map((x) => item('q', '?', esc(x.text), `${x.unverified ? '<span class="tagd unverified">unverified</span>' : ''}${cites(x.refs, s)}`)).join('')}` : ''}
    ...
    const answers = d.answers.map((a) => `<div class="qa"><div class="q">${esc(a.question)}</div><div class="a">${outdated(a) ? '<div class="hint amber">Outdated answer: the transcript changed. Ask again to update this answer and its citations.</div>' : ''}${esc(a.content)}${a.state === 'unanswerable' ? ' <span class="pill amber">No supporting passage</span>' : ''} ${cites(a.refs, a)}</div>${a.include_notes ? '<div class="hint">Your notes were included for this question.</div>' : ''}</div>`).join('');
    ...
    return `${body}
      ...
      ${draft ? `<div class="ask"><span class="eyebrow">Follow-up draft</span>${outdated(draft) ? '<div class="banner"><div class="body"><b>Outdated draft</b><p>The transcript changed. Regenerate this draft before relying on its text or timestamps.</p></div></div>' : ''}<textarea class="draft" data-draft-text spellcheck="false" style="margin-top:10px">${esc(draft.content)}</textarea><div class="actions" style="margin-top:10px"><button class="btn primary" data-copy-draft>${ico('copy', 14)}Copy</button><span class="hint">No send action. Paste it where you want it; timestamps in brackets point at passages.</span></div></div>` : ''}` : ''}`;
  }
